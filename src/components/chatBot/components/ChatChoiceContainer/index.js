...
class ChatChoiceContainer extends Component {
  state = {
    choices: [],
    renderedChoices: [],
    disabled: true,
  ...
  onChoicePress(choice) {
    if (
      this.props.multipleOptionsSelect == null ||
      this.props.multipleOptionsSelect === "false"
    ) {
  ...
  render() {
    ...
    if (this.props.multipleOptionsSelect === "true") {
      return (
        <View style={{ backgroundColor: "#F8F8F8" }}>
          <View style={styles.viewStyle}>{this.state.renderedChoices}</View>
          <Button
            style={{ margin: 5 }}
    ...
    return <View style={styles.viewStyle}>{this.state.renderedChoices}</View>;
  }
}
