...
class Chat extends React.PureComponent {
    ...
    initStatus(status) {
        ...
        const payloadToggleStatus = {
            ...
            toggleStatusParams: {
                ...
                updatedData: {
                    isActive: status,
                    toUserId: status ? expertData ? expertData.uid : questionData.expertInfo.uid : '',
                },
            },
            questionData,
        }
    ...
    componentDidUpdate() {
        ...
        if (questionValue && questionValue.isResolved && !questionValue.isRated && !isExpertRatingModalShow) {
    ...
    componentDidMount() {
        const { loadUserMessages, navigation, question, addQuestion, userData, setId, checkStatus, questionValue } = this.props;
        ...
        return (
            <View style={styles.headerStyle}>
                <TouchableOpacity
                    ...
                    onPress={() => { navigation.navigate(Constant.App.screenNames.BottomTab); }}>
                    <Image
                        style={{
                            width: 20,
                            height: 20,
                ...
                <View style={styles.profileHeaderStyle}>
                    <Avatar
                        ...
                        source={{
                            uri:
                                questionData && questionData.expertInfo.profileInfo.profileImageUrl ? questionData.expertInfo.profileInfo.profileImageUrl :
                                    expertData && expertData.profileInfo.profileImageUrl ? expertData.profileInfo.profileImageUrl
                                        : null
                        }}
                        activeOpacity={0.7} />
                    <CustomText style={styles.titleTextStyle}>
                        {questionData ? `${questionData.expertInfo.profileInfo.firstName} ${questionData.expertInfo.profileInfo.lastName}` :
                ...
                <View style={styles.ratingExpertModalParentContainerStyle}>
                    <View style={styles.ratingExpertModalInnerContainerStyle}>
                        ...
                        <Avatar
                            ...
                            size={100}
                            rounded
                            source={{ uri: expertData && expertData.profileInfo.profileImageUrl ? expertData.profileInfo.profileImageUrl : questionData && questionData.expertInfo.profileInfo.profileImageUrl ? questionData.expertInfo.profileInfo.profileImageUrl : null }}
                            activeOpacity={0.7} />
                        <CustomText style={styles.ratingExpertModalTextStyle}>
                            {`${lang.chat.subTitleRateExpert}${expertData ? expertData.profileInfo.firstName + ' ' + expertData.profileInfo.lastName : questionData.expertInfo.profileInfo.firstName + ' ' + questionData.expertInfo.profileInfo.lastName}`}
                        </CustomText>
                        <AirbnbRating
                            imageSize={40}
                            readonly={false}
                        ...
                        <CustomButton
                            ...
                            onPress={() => {
                                if (rating) {
                                    const expertValue = Object.assign({}, questionValue.expertInfo);
                                    if (expertValue.userRating && expertValue.userRating.length > 0) {
                                        ...
                                        expertValue.rating = parseInt((ratings / expertValue.userRating.length).toFixed());
                                    } else {
