...
export function getFilterDataWithCondition(obj) {
    try {
        ...
        return collection.get().then(querySnapshot => {
            ...
            const arr = [];
            querySnapshot.docs.forEach(element => {
                if (obj.professions && obj.professions.length > 0 && obj.languages && obj.languages.length > 0) {
...
export function sendEncryptedKeyToFirebase() {
    ...
    let Keyref = firebase.firestore().collection('EncryptedKeys').doc('EncryptedKeysDoc').get();
    return Keyref
        .then((doc) => {
            if (doc.exists) {
...
export const sendMessage = (obj) => {
    try {
        ...
        displayConsole('obj', obj);
        firebase.firestore().collection(Constant.App.firebaseTableNames.messages).doc(obj.id).collection('chat').doc().set(obj.messageParams);
