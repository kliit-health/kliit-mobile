...
function* changeUserPassword({ data }) {
    try {
        ...
        const responseReAunthenticate = yield reAunthenticate(params.currentPassword)
        ...
        if (responseReAunthenticate.success) {
            ...
            if (responseChangePassword.success) {
                ...
                //     newPassword: '',
                // });
                yield put(showOrHideModal(Lang.changePassword.passwordUpdateSuccessfullyMsg));
                navigation.goBack();
            } else {
            ...
            yield delay(500);
            yield put(showOrHideModal(
                responseReAunthenticate.message ? responseReAunthenticate.message
                    : Lang.errorMessage.serverError
            ));
        }
    } catch (error) {
